...
import { Menu, X, Globe } from './Icons';
  ...
  return (
    <header className={`citizen-header ${scrolled ? 'scrolled' : ''}`}>
      <div className="citizen-header-inner">
        ...
        <button className="hamburger" onClick={() => setMobileOpen(!mobileOpen)} aria-label={t('nav.menu')}>
          {mobileOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>

      <div className={`mobile-nav ${mobileOpen ? 'open' : ''}`}>
        <Link to="/" className={`citizen-nav-link ${isActive('/')}`} onClick={() => setMobileOpen(false)}>{t('nav.home')}</Link>
        <Link to="/map" className={`citizen-nav-link ${isActive('/map')}`} onClick={() => setMobileOpen(false)}>{t('nav.map')}</Link>
        <Link to="/track" className={`citizen-nav-link ${isActive('/track')}`} onClick={() => setMobileOpen(false)}>{t('nav.track')}</Link>
        <Link to="/about" className={`citizen-nav-link ${isActive('/about')}`} onClick={() => setMobileOpen(false)}>{t('nav.about')}</Link>
        <div className="mobile-lang-row">
          {LANGUAGES.map(l => (
            <button key={l.code} className={`mobile-lang-btn ${i18n.language === l.code ? 'active' : ''}`} onClick={() => { changeLang(l.code); }}>{l.label}</button>
          ))}
        </div>
        <Link to="/report" className="citizen-nav-cta" onClick={() => setMobileOpen(false)}>{t('nav.reportIssue')}</Link>
      </div>
    </header>
  );
};
