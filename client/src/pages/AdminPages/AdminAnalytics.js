  ...
  return (
    <AdminLayout>
      <div className="admin-page-header animate-fade-in-up">
        ...
        <p>Monitor system performance and water infrastructure trends</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="stats-grid animate-fade-in-up delay-1">
            <div className="stat-card blue">
              ...
              <div className="stat-card-label">Total Active Reports</div>
            </div>
            <div className="stat-card green">
              ...
              <div className="stat-card-label">Working Ratio</div>
            </div>
            ...
            <div className="stat-card purple">
              ...
              <div className="stat-card-label">Operational Points</div>
            </div>
          </div>
            ...
            <div className="card animate-fade-in-up delay-2">
              ...
              <div className="card-body">
                {[
                  { label: 'New Reports', value: stats.reports?.new_reports || 0, color: 'var(--primary)' },
                  { label: 'In Progress', value: stats.reports?.in_progress || 0, color: 'var(--warning)' }
                ].map(item => (
                ...
                <div style={{ padding: '0.875rem', background: 'var(--bg)', borderRadius: 'var(--radius-md)', textAlign: 'center', marginTop: '0.5rem' }}>
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>Total Active Reports</div>
            ...
            <div className="card animate-fade-in-up delay-3">
              <div className="card-header"><h3>Infrastructure Health</h3></div>
              <div className="card-body">
                {[
                  { label: 'Working Points', value: stats.water_points?.working || 0, color: 'var(--success)', bg: 'var(--success-light)', icon: <CheckCircle size={24} /> },
                  { label: 'Need Attention', value: stats.reports?.new_reports || 0, color: 'var(--danger)', bg: 'var(--danger-light)', icon: <AlertTriangle size={24} /> },
                  { label: 'Under Repair', value: stats.reports?.in_progress || 0, color: 'var(--warning)', bg: 'var(--warning-light)', icon: <Clock size={24} /> }
                ].map(item => (
          ...
          <div className="card animate-fade-in-up delay-4">
            <div className="card-header"><h3>System Insights</h3></div>
            <div className="card-body">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
                <div>
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 600, marginBottom: '0.375rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><TrendingUp size={16} /> Performance</div>
                  <p style={{ fontSize: '0.875rem', lineHeight: 1.7, margin: 0 }}>Average resolution time: {stats.avg_resolution_time_hours || 0} hours.</p>
                </div>
                <div>
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 600, marginBottom: '0.375rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Target size={16} /> Recommendation</div>
                  <p style={{ fontSize: '0.875rem', lineHeight: 1.7, margin: 0 }}>
                    {stats.reports?.new_reports > 5 ? 'High volume of new reports. Consider allocating more technicians.' : 'Report volume is manageable. Continue current operations.'}
                  </p>
                </div>
                <div>
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 600, marginBottom: '0.375rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Zap size={16} /> Current Focus</div>
                  <p style={{ fontSize: '0.875rem', lineHeight: 1.7, margin: 0 }}>
                    {stats.reports?.in_progress > 0 ? `${stats.reports.in_progress} repair${stats.reports.in_progress > 1 ? 's' : ''} in progress.` : 'All clear! No active repairs.'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="alert alert-error">Failed to load analytics. Please refresh.</div>
      )}
    </AdminLayout>
  );
};
